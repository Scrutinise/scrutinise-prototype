...
const RUMELT_SHAPE = [
  ...
  '  `instrument`     — which of the three routes the approach needs — legislative (a Bill or',
  '                     regulations), organisational (how a body works), financial (money) — or which',
  '                     combination, and why. Weigh all three; the proposer chooses between them.',
].join('\n')
...
const ENTITY_INSTRUCTION = [
  ...
  '',
  'The person who wrote the account below does not know these words. They wrote a plain complaint;',
  'the field has a name for the office, the doctrine, the regime and the power that bear on it. THOSE',
  'are what we want, for THIS subject. Give the name as it is actually used — the proper name of the',
  'principle, not a paraphrase of what it does.',
  '',
...
export async function critiqueKernel(input: {
  ...
  const system = [
    ...
    '',
    '⚠⚠ THE THREE AVENUES ARE NOT YOURS TO CHOOSE BETWEEN. The kernel evaluates a legislative, an',
    'organisational and a financial route, each to the same depth, and the proposer decides which to',
    'take. In `rewrite` you may sharpen the plan; you may NOT drop, defer or demote the legislative',
    'route (or either other) because it is harder — "hard" is a finding to state, not a reason to',
    'write it out. If you think one avenue is the wrong one, say so in `forkDoubts` with forkKey',
    '"actions:avenue" and the reason, and leave the avenue standing.',
    '',
    TESTIMONY_INSTRUCTION,
  ].join('\n')
