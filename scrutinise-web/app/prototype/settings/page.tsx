...
export default function SettingsPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-10">
      <div className="mb-8">
        <Link href="/prototype/dashboard" className="text-xs text-primary hover:underline transition-colors">
          ← Dashboard
        </Link>
        <h1 className="text-2xl font-bold text-foreground mt-4">Account Settings</h1>
        <p className="text-sm text-muted-foreground mt-1">Manage your profile, privacy, and preferences.</p>
      </div>

      {/* ── Section 1: Account ── */}
      <section className="mb-8">
        <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-4">Account</h2>
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex items-center rounded-md border border-border bg-background overflow-hidden focus-within:ring-2 focus-within:ring-ring/50 transition-colors">
              <span className="pl-3 text-muted-foreground text-sm">@</span>
              <input
                type="text"
                value={username}
                onChange={e => setUsername(e.target.value)}
                className="flex-1 bg-transparent px-2 py-2 text-foreground placeholder:text-muted-foreground text-sm focus:outline-none"
              />
            </div>
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Email <span className="text-muted-foreground/60">(managed by Clerk)</span></label>
            <input
              type="email"
              value="user@example.com"
              disabled
              className="w-full rounded-md border border-border bg-muted px-3 py-2 text-muted-foreground text-sm cursor-not-allowed"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Professional expertise</label>
            <input
              type="text"
              value={expertType}
          ...
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Political party <span className="text-muted-foreground/60">(optional)</span></label>
            <input
              type="text"
        ...
        <button className="mt-4 px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-md text-sm font-semibold transition-colors">
          Save changes
        </button>
      </section>
      ...
      <section className="mb-8">
        <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-4">Status Claims</h2>
        ...
        {parliamentaryModal === 'open' && (
          <div className="mt-4 bg-card border border-border rounded-lg p-5 space-y-4">
            <h3 className="text-sm font-semibold text-foreground">Claim Parliamentary Status</h3>
            <div>
              ...
              <div className="flex gap-3">
                {(['MP', 'Lords'] as const).map(r => (
                  <label key={r} className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-foreground">{r === 'Lords' ? 'Member of the House of Lords' : 'MP'}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            {parliamentaryRole === 'Lords' && (
              <div>
                <label className="block text-xs text-muted-foreground mb-1">Peerage title</label>
                <input
                  type="text"
                  value={peerage}
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Parliament.uk profile URL</label>
              <input
                type="url"
                value={parliamentUrl}
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-5 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-md text-sm font-semibold transition-colors"
              >
                Submit claim
              </button>
              <button
                onClick={() => setParliamentaryModal('closed')}
                className="px-4 py-2.5 border border-border bg-background text-foreground hover:bg-accent rounded-md text-sm transition-colors"
              >
                Cancel
              </button>
            </div>
        ...
        {professionalModal === 'open' && (
          <div className="mt-4 bg-card border border-border rounded-lg p-5 space-y-4">
            <h3 className="text-sm font-semibold text-foreground">Claim Professional Status</h3>
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Firm or Chambers</label>
              <input
                type="text"
                value={firmName}
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Professional credentials</label>
              <input
                type="text"
                value={credentials}
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Licence or bar number <span className="text-muted-foreground/60">(optional)</span></label>
              <input
                type="text"
                value={licenceNumber}
            ...
            <div>
              <label className="block text-xs text-muted-foreground mb-1">Supporting document (PDF, max 5MB)</label>
              <input
                type="file"
                accept=".pdf"
      ...
      {/* ── Section 3: Privacy ── */}
      <section className="mb-8">
        <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-4">Privacy</h2>
        <div className="space-y-3">
          <button className="px-4 py-2.5 border border-border bg-background text-foreground hover:bg-accent rounded-md text-sm font-medium transition-colors">
            Download my data
          </button>
          <div>
            <button className="px-4 py-2.5 border border-red-800 text-red-400 hover:border-red-600 hover:text-red-300 rounded-md text-sm font-medium transition-colors">
              Delete my account
            </button>
            <p className="text-xs text-muted-foreground mt-2">Permanently deletes all your ideas, votes, and contributions. This cannot be undone.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mb-8">
        <h2 className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-4">Notifications</h2>
        <div className="space-y-4">
          {/* Global toggle */}
          <div className="flex items-center justify-between p-4 bg-card border border-border rounded-lg">
            <div>
              <p className="text-sm text-foreground font-medium">Email notifications</p>
              <p className="text-xs text-muted-foreground mt-0.5">Receive email alerts for the events below</p>
            </div>
            <button
              onClick={() => setGlobalEmailToggle(v => !v)}
              className={`w-10 h-6 rounded-full transition-colors flex-shrink-0 ${globalEmailToggle ? 'bg-primary' : 'bg-muted'}`}
            >
          ...
          {(Object.keys(notifToggles) as Array<keyof typeof notifToggles>).map(key => (
            <div key={key} className="flex items-center justify-between">
              ...
              <button
                ...
                className={`w-10 h-6 rounded-full transition-colors ${notifToggles[key] && globalEmailToggle ? 'bg-primary' : 'bg-muted'}`}
