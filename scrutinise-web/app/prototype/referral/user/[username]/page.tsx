    ...
    <div className="min-h-screen bg-gray-950 text-white">
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center gap-5 mb-8">
          <div className="w-16 h-16 rounded-full bg-revolutBlue flex items-center justify-center text-xl font-bold text-white flex-shrink-0">
            {initials}
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              {user.constituency && (
                <span className="text-xs text-gray-400">{user.constituency}</span>
              )}
            </div>
            <div className="mt-2">
              <span className="text-xs text-gray-500">Credibility Score: </span>
              <span className="text-sm font-semibold text-white">{user.credibility.toLocaleString()}</span>
            </div>
          </div>
        </div>
        ...
        {userIdeas.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Their ideas</h2>
              ...
              {userIdeas.map(idea => {
                ...
                return (
                  <Link
                    ...
                    className="block p-4 bg-gray-900 border border-gray-700 rounded-xl hover:border-gray-500 hover:bg-gray-800 transition-all"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-400 mt-1 line-clamp-2">{idea.summary}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <div className="text-sm font-bold text-white">{total.toLocaleString()}</div>
                        <div className="text-xs text-gray-500">votes</div>
                        <div className="text-xs text-gray-500 mt-1">Passion: {idea.passionScore}</div>
                      </div>
                    </div>
                  </Link>
                )
        ...
        <div className="p-5 bg-blue-950/30 border border-blue-800 rounded-xl text-center">
          ...
          <p className="text-xs text-gray-400 mb-4">Free to join. Your vote shapes real policy.</p>
