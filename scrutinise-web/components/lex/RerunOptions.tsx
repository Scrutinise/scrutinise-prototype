  ...
  return (
    <div>
      ...
      {/* ══ BRIEF_26E §5 — THE SAME COLLAPSING HEADER AS THE KERNEL SECTIONS ═════════════
          ...
          exists at all (see the file header). It used to sit permanently open while every
          section around it in the panel could collapse. `CollapsedSection` IS that control —
          its own header comment says it copies the kernel headings' vocabulary exactly. Open
          by default while a build is actually running, so a live status is never hidden. */}
      <CollapsedSection
        title="Re-run"
        defaultOpen={running}
