...
*Produced by CCh — save to docs/ before issuing to CC* *Date: 10-03-26*
...
Replace the CSS custom property block in `scrutinise-web/app/globals.css` with the full token set from `docs/v0-export/app/globals.css`. Keep any Scrutinise-specific additions already in the file. Copy across exactly:
...
Source: `docs/v0-export/components/ui/`
...
Use `docs/v0-export/app/dashboard/page.tsx` as layout and spacing reference only. Do not replace mock data, routing, or any existing content.
...
*CC_briefing_v0_design_FINAL.md — produced by CCh — 10-03-26* *Save to docs/ before issuing to CC*
