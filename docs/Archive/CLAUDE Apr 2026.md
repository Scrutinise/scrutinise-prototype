...
Keys: {legislationGovUkId}/sections/{N}.xml           (raw CLML)
      {legislationGovUkId}/sections/{N}.compiled.txt  (compiled text — TNA or AI)
      {legislationGovUkId}/sections/{N}.summary.txt   (Lex plain-English summary)

On-demand flow: FTS query Railway → R2 key → r2Get() → user.
Analytics flow: copy batch R2→Railway, run analysis, delete source.
