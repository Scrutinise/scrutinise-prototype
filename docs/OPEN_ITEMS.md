...
| **OI-17** | **Neon is past its alert line, and was before 25-H added to it.** `pg_database_size` reads **18 GB** against the 17.5 GB line recorded in `setup-edges-table.ts`; `citation_edge` adds ~0.6 GB on top of `legislation_edges`' 0.94 GB. | `pg_database_size` = 18 GB, alert line 17.5 GB, measured 2026-08-26. | Both are droppable and neither is load-bearing for the live site. **Whether the headroom is acceptable is Charlie's call, not a sprint's** — recorded here so the next volume-adding sprint reads it first. |
...
*(none yet — this register was created on 2026-08-24)*
