...
the decision log (§13, 6 Aug afternoon); this section is how they behave. Full account: `CHANGE_LOG.md`
...
## 9. Stage 2e — pilot polish, and Stage 2c's attribution item at last **[BUILT 24 Aug 2026]**

Executes the "Central Stage 2e" brief and its addendum (24 Aug 2026), after Charlie's browser walk of
2d. Schema: `prisma/central_stage2e.sql`, hand-written and applied to Neon.
`npm run check:central` — **344/344**, up from 295.

### 9.1 The audit, before anything was changed

The brief asked for a diagnosis of the points failure before a fix. Three findings, from the live
database:

1.  **An answer vote was never wired to the ledger at all.** `recordPointsEvent` had exactly three
    call sites, all in `lib/central-points.ts`: two for bulletin marks and one for claim approval.
    `setAnswerVote` wrote an `AnswerVote` row and stopped. Stage 2b built the vote as a ranking
    signal; Stage 2 built the ledger for bulletin marks; **nobody joined them.** Not a regression —
    it was never built.
2.  **`PointsEvent` held zero rows across the whole database.** Nothing had ever paid a point in
    Central, by any route.
3.  **"Log this session" worked exactly as designed.** One `TrainingSession`, two `ActivityClaim`
    rows, both PENDING, both correct. The points were not missing; they were waiting for an approval
    nobody had told Charlie to give. That is what item 8 removes.

### 9.2 Item 0 — AI attribution (the launch blocker, briefed 12 Aug, missed)

`Answer.authorType` (MEMBER | AI) and nullable `Answer.aiModel`. Backfill keyed on the seed account's
`clerkId`, never a hard-coded id: **27 answers before, 27 after**, and 4 member answers untouched.

**The specific defect:** the answer card rendered *no author at all* — a branch name and an age. All
27 Claude-written answers were indistinguishable from members' work on every screen. One component,
`components/central/AnswerByline.tsx`, now answers "who wrote this", and three surfaces use it:
the library list (label above the preview text, because the list is the one place most people ever
look), the question detail, and the pack. `check:central` greps all three, so a fourth surface that
forgets is a failing check.

The printed sheet uses a **plain-text** variant — "Written by Claude — not a member's answer" —
because a coloured badge is the first thing a printer loses, and the pack is the artefact a member
reads off in a room with no context to explain it.

**An AI answer mints no points.** A vote on one still ranks it — members should be able to say which
answer is best regardless of what wrote it — and writes no `PointsEvent`, so the seed account cannot
accrue points for content nobody wrote. It does not spend the voter's daily budget either.

### 9.3 Item 1 — the template

⚠ **Stage 2d's template was generated by SheetJS and Excel offered to repair it**, which stripped the
formatting, the example rows and the Context drop-down. The generator is deleted.
`public/central-question-upload-template.xlsx` is now Charlie's file, and
`scripts/patch-question-template.ts` edits its XML **in place** — one worksheet part and the shared
string table — copying every other zip entry through byte for byte. `styles.xml` and the Questions
sheet come out byte-identical, the `x14:dataValidation` that IS the Context drop-down is untouched,
and `count`/`uniqueCount` on the string table are maintained separately (setting both to the same
number is itself a repair prompt).

The 24 UK **ministerial** departments are added to the Topics column — not the ~400 agencies, which
would make the list unusable — and seeded as unpromoted `QuestionTag` rows so the library already
knows them. ⚠ **Consequence for Charlie:** the "All topics" dropdown is now 35 entries per node
rather than 11.

The validator is aligned to the file exactly: the Notes column is headed **`Notes (not imported)`**;
the questions are read from the sheet **named** `Questions`, never `SheetNames[0]` (which is
"Read me first" — reading sheet one would have parsed the instructions and reported no Question
column); and the template's own guidance row and three `EXAMPLE — delete this row.` rows are
recognised and reported as skipped with a reason, rather than failing on every column at once.

### 9.4 Items 2–5 — the walk's UI findings

-   **Upload** — the browser's grey "Choose File" is replaced by a primary Button driving a hidden
    input, with the chosen filename on its own line under **File selected**.
-   **Vote affordance** — one line by the count: "▲ You get one vote per question — click it again to
    take it back."
-   **Training** — both panels expose the proposal link, labelled **"See 1 proposal" / "See 3
    proposals"**; an optional message box sits on **both** Accept and Decline, stored on the match
    and delivered in the notification, because a silent refusal in a branch of a dozen people is
    worse than no listing at all; and the feed line names the person and the listing —
    *"{Name} has proposed on your offer of {topic}"*.

### 9.5 Item 3 — answer votes reach the ledger

`applyAnswerVote` composes guardrails → vote → ledger, mirroring `applyBulletinMark`, and
`lib/question-library.ts` still never imports the points engine. Same two tariffs
(MARK_CONSTRUCTIVE +4 / MARK_UNCONSTRUCTIVE −4), same `MARK_RECEIVED`/`MARK_REMOVED` event types,
**one shared daily budget across both surfaces** — the limit is on how far one member can move other
members' scores in a day, not on which page they did it from. Only `sourceType` differs, so the two
remain tellable apart in the ledger. Switching a vote reverses the old award **at the value the
original used** and pays the new one; nothing is ever updated in place.

`scripts/backfill-answer-vote-points.ts` pays votes cast before the wiring existed — through
`recordPointsEvent`, so the referral chain mints from them as it would have. One vote qualified.

### 9.6 Item 8 — activity pays immediately, and can be reversed

**Charlie's decision, 24 Aug: pre-approval is removed.** A claim and a logged training session pay
their tariff on submission. A manager of that node or any node above it may **reverse** an award,
with a **required** reason the claimant is told; the reversal is a second `PointsEvent` at the value
the original award used, never today's tariff, and both rows stay — the ledger only appends.

⚠ **A bug this sprint created and caught in its own check:** `logSessionForMatch` builds its two
claims directly rather than through `createActivityClaim`, because it raises the *other*
participant's claim. Under the old model both routes produced a PENDING row and a manager paid it;
with the gate gone, the direct route paid nothing and said nothing. Both now go through one
`awardClaimPoints`.

The one-per-day guard's predicate widens from `status <> 'DECLINED'` to
`status NOT IN ('DECLINED','REVERSED')`: a reversal says the claim should not have paid, so the
member has to be able to put it right.

**The Community activity log is now load-bearing.** With nothing standing between a claim and the
points, the fact that every award is witnessed by the whole Community and reversible with a stated
reason is the entire anti-abuse mechanism. It is written up as such on the page itself.

The Teams tab carries a count badge of what genuinely awaits a manager — join requests, plus anything
the old model left pending. ⚠ **Not a claims queue**: after item 8 there is none, and a badge that
could only ever read zero is worse than no badge.

### 9.7 Item 9 — referral accrual, and the hazard register

**The chain paid nothing and the reason was arithmetic.** Bonuses were `Math.floor(points × rate)`,
and 10% of a 4-point mark floors to zero — so marks paid the chain nothing at all, and raising the
mark value would only have moved the threshold. Each `CommunityReferral` link now carries a
`bonusBalance` decimal and mints a whole event when it crosses 1.0. Ten 4-point marks pay the L1
inviter **4**, where flooring paid **0**; a single mark still pays nothing *yet*, and the check
asserts the 0.4 is held on the link rather than discarded.

`docs/CLAUDE.md` §21 is the standing **"indexes Prisma can't see"** register: both partial/expression
unique indexes in Central, what each guards, that `migrate diff` will propose dropping them, and the
rule that `check:central` asserts their predicates. It also records that `prisma format` must never
be run on the schema.

### 9.8 Carried forward

-   **Events — the original Stage 2c scope — stays deferred.** Not needed for the pilot (§7).
-   Phone sharing stays on. Charlie used it end to end; the question 2d raised is answered.

## 10. Stage 3 — Corpus-powered AI features **[DESIGNED]**
...
## 11. Stage 4 — Token economy **[ROADMAP — accountant question first]**
...
-   **24 Aug (evening)** — Charlie walked 2d in the browser; Stage 2e briefed and built (§9). Nine
    decisions:
    1.  **Pre-approval is removed.** An activity claim and a logged training session pay on
        submission. Speed for members; accountability kept through visibility plus reversal.
    2.  **Reversal replaces approval** — managers only, a reason REQUIRED, at the original award
        value, both events left in the ledger.
    3.  **The activity log becomes load-bearing.** With no gate in front of the points, the fact that
        every award is witnessed and reversible IS the anti-abuse mechanism.
    4.  **Answer votes pay, mirroring bulletin marks** — same tariffs, same event types, ONE shared
        daily budget across both surfaces. They had never been wired to the ledger at all.
    5.  **AI answers are labelled everywhere and mint nothing.** A vote still ranks them; no
        PointsEvent is written, so the seed account cannot accrue points for content nobody wrote.
    6.  **Referral bonuses accrue a fraction.** Flooring 10% of a 4-point mark paid the chain zero;
        each link now carries a decimal and mints on crossing 1.0.
    7.  **The template is Charlie's file, edited in place, never regenerated.** A SheetJS-written
        workbook made Excel offer to repair it and stripped the drop-down.
    8.  **Topic list gains the 24 ministerial departments** — not the agencies, which would make the
        dropdown unusable.
    9.  **A decline may carry a line of explanation**, because a silent refusal in a branch of a
        dozen people is worse than no listing at all.
