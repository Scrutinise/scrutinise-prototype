...
**Rule that fixes the lingering card:** the confirmation card is rendered if and only if
`field.status === AWAITING_CONFIRMATION`. On accept, the server flips the status to `ACCEPTED`, returns
the new canonical state, and the card disappears because the state no longer says to show it. The next
...
|---|---|---|
| **1 — Chat** | Message history + Lex's `chatText` + the accept card when a field is `AWAITING_CONFIRMATION` | messages + `currentField` |
| **2 — Fields** | All pages, all fields, statuses, values, "X of Y" per page; green tick on `ACCEPTED` | `pages[]` |
