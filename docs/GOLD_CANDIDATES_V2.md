...
**Status:** ✅ **VALIDATED BY CHARLIE, 22 AUGUST 2026 — 24 of 24 reviewed: 22 ACCEPT and 2 AMEND, both amendments applied.** This set is now scoreable. The machine-readable transcription is `scrutinise-web/scripts/gold/gold-v2-set.ts`, and `npm run check:goldv2` asserts the two agree so they cannot drift.

⚠ **The two amendments were NOT "accept with a note" and are not recorded as accepts.** Q6 "hanging" → "the death penalty"; Q12 "make me leave" → "evict me". Q6's amendment also **reclassifies it**: "the death penalty" is the document's own wording, so it is no longer one of the vocabulary-avoided questions and that count drops **9 → 8** (§3 requires ≥3). Recorded rather than absorbed.
...
|---------|---------------------------------------------|-------|----------|
| Q1–Q11  | Debates                                     | 11    | ✅ done   |
| Q12–Q21 | Legislation                                 | 10    | ✅ done   |
| N1–N3   | Negative controls — **a 0% here is a PASS** | 3     | ✅ done   |
...
| questions that deliberately avoid the document's own vocabulary | **8** (§3 requires ≥3) — was 9 before Q6's amendment |
| **VERDICTS: ACCEPT** | **22** |
| **VERDICTS: AMEND (applied)** | **2** — Q6, Q12 |
| **VERDICTS: REJECT** | **0** |
...
2.  ⚠⚠ **RETRACTED 2026-08-22 — THIS CLAIM WAS WRONG, AND THE QUERY THAT MADE IT IS THE FINDING.** The original text read: *"three obvious candidates are simply absent — the `Vagrancy Act 1824`, the `National Minimum Wage Act 1998` and the `Housing Act 1996` are all not in the corpus… a user asking 'is it illegal to sleep rough?' cannot be answered by this corpus at all."* **All three are held, all compiled, and all three are returned by `runSearch()` today.** Housing Act 1996 — **532 sections**, rank 0. National Minimum Wage Act 1998 — **108 sections**, rank 0. Vagrancy Act 1824 — **20 sections**, and its two operative offences (s.3 begging, s.4 rogue and vagabond) carry full text; it comes back at rank 20 tier-scoped and rank 110 on the lay query, so it is *badly ranked*, not absent. The mistake was keying the lookup on the **calendar id** `ukpga/1824/83` while the corpus holds the Act under its **regnal id** `ukpga/Geo4/5/83` (5 Geo 4 c 83) — the exact trap `v36-reconcile.ts` was written to close, and which its own reconciliation had already handled correctly (`ukpga/1824`: present 4, *all four* via the regnal id). **The 21.4% figure did not support the claim either**: it is instrument-level coverage of pre-2000 Acts, and cannot say anything about three named ones. ▶ **"An old Act still in force" is therefore an archetype that CAN be keyed** — the Vagrancy Act is a live candidate, and its ranking is a retrieval finding worth a question of its own.
...
### Q6 · When did Parliament last seriously debate bringing back the death penalty?
...
-   **Shape:** U — when last considered · **Sourced:** document-outward · ⚠ **vocabulary: NOT avoided (changed by the amendment)**
-   ⚠ **This is the only question testing** `historic-hansard` **(4.6M sections), the second-largest collection in the corpus.**
-   ⚠⚠ **THE AMENDMENT CHANGES WHAT THIS QUESTION TESTS, AND THE COUNT MOVES WITH IT.** As drafted it said "hanging" and was one of the nine questions deliberately phrased in words the document does not use. "The death penalty" **is** the document's own wording (`DEATH PENALTY (ABOLITION) BILL`), so Q6 is no longer a vocabulary-avoided question and the count drops **9 → 8**. Charlie's wording is the right call — nobody types "bringing back hanging" into a search box — but the reclassification is recorded rather than left to make the ≥3 requirement look larger than it is.
-   **VERDICT →** **AMEND — applied 22 Aug 2026: "hanging" → "the death penalty"**
...
### Q12 · Can my landlord evict me without giving a reason?
...
-   **Shape:** X — which law governs this · **Sourced:** outside-in · **vocabulary: avoided** (still — "evict" is the everyday word; the provision says *"recovery of possession on expiry or termination of an assured shorthold tenancy"*)
-   **VERDICT →** **AMEND — applied 22 Aug 2026: "make me leave" → "evict me"**
