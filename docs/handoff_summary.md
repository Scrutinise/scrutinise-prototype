...
- **STEP 2 BLOCKED at the Batch tier:** account = paid Tier 1 → **500k enqueued-token queue**
...
- **Billing decode (Charlie):** £150 payment = CREDIT not spend; usage ≈ $36; Tier 2 = ≥$100 ACTUAL
  ...
  the slice fits. Tier-2 monthly cap $2,000 → remainder fits same month.
- **BUILT INERT this sprint (offline-selftested, tsc clean):** `VECTOR_EMBED_MODE=sync|batch` —
  sync drives the SAME 12k shard plan + `corpus_vec` writes through NEW `gemini-sync.ts`
  (embedContent at $0.15/1M, ≤100 texts/≤18k tok per call, GLOBAL 950k-TPM/2,800-RPM pacer,
  429 retries); checkpoint pins `shardSize` across the transport switch; `gemini-batch.ts` splits
  over-budget shards into sequential ≤4.5M-tok sub-jobs (dense caselaw/debate regions ~800 tok/chunk
  would otherwise 429 at Tier 2 forever); `gemini-tier-probe.ts` = ~free tier detector.
- **THE GATED PLAN:** (0) sync `--canary` ~$0.03 → (1) sync slice on cpx62, console-watched to
  ≥$100 usage (~667M tok, ~11.7h; ~$64/7.5h if the $36 counts) → (2) probe confirms Tier 2 →
  (3) relaunch batch `VECTOR_SHARD_SIZE=12000 VECTOR_MAX_INFLIGHT=1` (~1,900–2,200 sequential jobs,
  days-to-2-weeks) → ANN (32GB risk → `--index-only` on CCX43 fallback). **Revised total ~$470–560**
  (~$50 sync premium buys the flip), under the ~$600 gate. **DO NOT start any embed without Charlie.**
