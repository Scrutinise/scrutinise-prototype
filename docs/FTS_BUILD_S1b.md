...
1. **Append + resume-delete (correctness).** Batches are **appended** (`tbl.add`), not upserted. mergeInsert was the throughput bottleneck — the S1b canary measured Railway ≈ home (~123 rows/s) because the build is **write-bound, not bandwidth-bound**, and merge cost grows with table size. Idempotency instead comes from the cursor: on resume the indexer first **deletes any rows with `id > lastId`** (a crash can leave an appended-but-un-checkpointed batch), then appends `WHERE id > lastId`. A death after a Lance write but before the checkpoint save can never leave duplicates — the stray tail is dropped.
...
The cursor drives both: the optimisation (skip done work) AND, via the resume-delete, the correctness guarantee (no dupes). Together: a run that dies at hour 5 resumes; a re-trigger from the checkpoint is still safe. **`--reset` is a DISCRETE step** (`--reset-only` drops+recreates the empty table, writes a fresh checkpoint, exits) — deliberately NOT baked into the Railway start command, because the `ON_FAILURE` restart policy re-runs that command on every crash and a `--reset` there would wipe progress. `--limit N` loads the first N only (no index); `--canary` does an isolated N-row load **and** index build into `corpus_fts_canary` (via `FTS_TABLE_NAME`).
