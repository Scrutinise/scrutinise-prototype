...
async function main() {
  ...
  for (const corpus of ['hmrc-ancillary', 'tax-treaties-dta', 'hmrc-manuals']) {
    const open = await pool.query(`SELECT count(*)::int n FROM ingest_queue WHERE corpus=$1 AND status IN ('pending','claimed','blocked','failed')`, [corpus])
    ...
    if (open.rows[0].n > 0) { console.log(`${corpus}: ${open.rows[0].n} open rows — NOT re-baselining`); continue }
